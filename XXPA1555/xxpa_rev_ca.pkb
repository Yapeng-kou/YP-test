...
CREATE OR REPLACE PACKAGE BODY apps.xxpa_rev_ca 
AS
/*********************************************************
** Title:       XXPA1555 Extension for Term Service Contract Revenue Recognition
** File:        xxpa_rev_ca.pkb
** Description: This script creates a package body. Procedure Auto_rev_event_gen() creates
**              automatic events for Revenue Recong.
** Parameters:  {None.}
** Run as:      APPS
** Keyword Tracking:
...
** 30-Jan-2013   Jaya Gupta         Changes for CR5944.
** 13-Nov-2013   Jaya Gupta		      Fix RT 5058267.
** 20-Dec-2013   Jaya Gupta		      Fix RT 4884352.
** 24-Apr-2015   Jaya Gupta         CR22722 - When Funding Level is Top Task, Work Type at Task needs to be Straight Line Revenue.
** 09-Sep-2015   Mriganka Purkayastha CR22995 - Update Service Revenue Extension to use DFF on Task Level Work Type to determine Revenue
** 04-Dec-2015   Archana/ Harish Chintakuntla  Defect#20484-- There is an issue in the Client extension code, for project level funding scenario if there is no default task defined in the lookup SERVICE_REV_OU_TASK_MAPPING
** 10-Oct-2017   Santhosh Ramakrishnan Defect#26090 - Service Agreement: Revenue not generated for project 154034788 task 10
** 12-Jun-2023   Kasthuri AR        TASK0662588 - INC0647629 - Hard Limit Error (Additional Events created by Client extension with wrong amount) 
********************************************************/
  /*****************************************************************************
...
BEGIN
  if G1_DEBUG_MODE = 'Y' then
    xx_au_debug.p_write('=====begin xxpa_rev_ca.get_rev_budget_amount======');
    xx_au_debug.p_write('=====Params get_rev_budget_amount X2_project_id              : '||X2_project_id);
    xx_au_debug.p_write('=====Params get_rev_budget_amount X2_task_id                 : '||X2_task_id);
    xx_au_debug.p_write('=====Params get_rev_budget_amount X2_agreement_id            : '||X2_agreement_id);
    xx_au_debug.p_write('=====Params get_rev_budget_amount P_rev_budget_type_code     : '||P_rev_budget_type_code);
    xx_au_debug.p_write('=====Params get_rev_budget_amount P_rev_plan_type_id         : '||P_rev_plan_type_id );
    xx_au_debug.p_write('=====Params get_rev_budget_amount X_rev_date                 : '||X_rev_date );
  END IF;
  X_status        := 0;
  ...
  END IF;
  IF g1_debug_mode = 'Y' THEN
    xx_au_debug.p_write(' 1 l_rev_budget_type_code '||l_rev_budget_type_code||' l_rev_plan_type_id '||l_rev_plan_type_id);
  END IF;
  BEGIN
    SELECT 'x'
    INTO dummy
    ...
    IF g1_debug_mode = 'Y' THEN
      xx_au_debug.p_write('2 dummy '||dummy);
    END IF;
  EXCEPTION
  ...
  if g1_debug_mode          = 'Y' then
    xx_au_debug.p_write(' Fetching project start and end date.');
  END IF;
  ...
  if g1_debug_mode          = 'Y' then
    xx_au_debug.p_write(' l_prj_end_dt : '||l_prj_end_dt||' l_prj_start_dt : '||l_prj_start_dt);
  END IF;
  ...
  IF g1_debug_mode        = 'Y' THEN
    xx_au_debug.p_write('22  X2_revenue_amount '||X2_revenue_amount||' X_rev_budget_type_code '||X_rev_budget_type_code);
    xx_au_debug.p_write('=====end xxpa_rev_ca.get_rev_budget_amount======');
  END IF;
EXCEPTION
WHEN invalid_funding_budget THEN
  ...
  X_status               := l_status;
  pa_billing_pub.insert_message (X_inserting_procedure_name =>'xxpa_rev_ca.get_rev_budget_amount', X_attribute1 => l_cost_budget_type_code, X_attribute2 => l_rev_budget_type_code, X_message => status, X_error_message=>status, X_status=>l_status);
WHEN OTHERS THEN
...
IS
  ...
  l_prjt_fund_amount  NUMBER;  --INC0647629
...
BEGIN
  IF g1_debug_mode = 'Y' THEN
    xx_au_debug.p_write(' =====START : Params Auto_rev_event_gen X_request_id       : '||X_request_id );
    xx_au_debug.p_write('     =====begin xxpa_rev_ca.Auto_rev_event_gen======     ');
    xx_au_debug.p_write(' =====Params Auto_rev_event_gen X_project_id               : '||X_project_id);
    xx_au_debug.p_write(' =====Params Auto_rev_event_gen X_top_task_id              : '||X_top_task_id);
    xx_au_debug.p_write(' =====Params Auto_rev_event_gen X_calling_process          : '||X_calling_process);
    xx_au_debug.p_write(' =====Params Auto_rev_event_gen X_calling_place            : '||X_calling_place );
    xx_au_debug.p_write(' =====Params Auto_rev_event_gen X_amount                   : '||X_amount );
    xx_au_debug.p_write(' =====Params Auto_rev_event_gen X_percentage               : '||X_percentage );
    xx_au_debug.p_write(' =====Params Auto_rev_event_gen X_rev_or_bill_date         : '||X_rev_or_bill_date );
    xx_au_debug.p_write(' =====Params Auto_rev_event_gen X_billing_assignment_id    : '||X_billing_assignment_id );
    xx_au_debug.p_write(' =====Params Auto_rev_event_gen X_billing_extension_id     : '||X_billing_extension_id );
  END IF;
  ...
  IF g1_debug_mode   = 'Y' THEN
    xx_au_debug.p_write(' l_curr_rev_eve_dt: '||l_curr_rev_eve_dt||' l_last_rev_eve_dt: '||l_last_rev_eve_dt);
  end if;
  ...
  IF G1_DEBUG_MODE            = 'Y' THEN
    xx_au_debug.p_write(' Fetching project details.');
  END IF;
  if g1_debug_mode = 'Y' then
    xx_au_debug.p_write(' No of days between prj start and end date :'||to_char(l_prj_no_of_days));
    xx_au_debug.p_write('prj start dt:'||TO_CHAR(L_PRJ_START_DT)||' and prj end date :'||TO_CHAR(L_PRJ_END_DT)||' prj_fund_lvl : '||L_PRJ_FUNDING_LVL);
  END IF;
  ...
  END;
   IF g1_debug_mode   = 'Y' THEN
    xx_au_debug.p_write(' l_lookup_task: '||l_lookup_task||' l_lookup_found: '||l_lookup_found);
  end if;
  if l_lookup_found = 'Y' then
    ...
    exception
      WHEN NO_DATA_FOUND THEN
         ...
         xx_au_debug.p_write(' prj_tsk_err ');
          RAISE prj_tsk_err ;
      WHEN TOO_MANY_ROWS THEN
      xx_au_debug.p_write(' prj_too_many_tsk ');
          raise prj_too_many_tsk;
      WHEN OTHERS THEN
      xx_au_debug.p_write(' arbit_err ');
          raise arbit_err;
    end;
  else
  ...
  END IF;
  IF g1_debug_mode   = 'Y' THEN
    xx_au_debug.p_write(' l_lookup_task_id: '||l_lookup_task_id);
  end if;
  ...
  IF  l_prj_funding_lvl = 'PROJECT' then
     ...
     END IF ; --l_lookup_found ='Y'
          ...
          IF g1_debug_mode = 'Y' THEN
            xx_au_debug.p_write(' IN Params passed to xxpa_billing_pub.get_rev_budget_amount : ');
            xx_au_debug.p_write(' Params X_project_id : '||X_project_id);
            xx_au_debug.p_write(' Params X_top_task_id : '||L_TOP_TASK_ID);
            xx_au_debug.p_write(' Params l_rev_plan_type_id : '||l_rev_plan_type_id);
            xx_au_debug.p_write(' Params X_rev_date : '||TO_CHAR(l_curr_rev_eve_dt));
          END IF;
          xxpa_rev_ca.get_rev_budget_amount( x2_project_id => x_project_id,
          ...
          END IF;
          IF g1_debug_mode = 'Y' THEN
            xx_au_debug.p_write(' OUT values from xxpa_billing_pub.get_rev_budget_amount : ');
            xx_au_debug.p_write(' Params X2_revenue_amount : '||budget_revenue);
            xx_au_debug.p_write(' Params X_rev_budget_type_code : '||l_rev_budget_type_code);
          END IF;
          IF budget_revenue  = 0 THEN
            IF g1_debug_mode = 'Y' THEN
              xx_au_debug.p_write(' No budget for the project ');
            END IF;
            RAISE NO_FUNDIND_ERR;
          END IF;
          IF g1_debug_mode = 'Y' THEN
            xx_au_debug.p_write(' budget_revenue amt : '||budget_revenue||' for project '||X_project_id);
            xx_au_debug.p_write(' formula applied rev amt per day = '||budget_revenue||' divided by '||l_prj_no_of_days);
          END IF;
        TOT_REV_EVE_AMT  :=0;
        TOT_EVE_AMT2     :=0;
        TOT_L_REV_AMT    :=0;
        l_revamt_eve_crtd:=0;
		l_prjt_fund_amount:=0;  --INC0647629
        ...
        LOOP
          IF g1_debug_mode = 'Y' THEN
            xx_au_debug.p_write(' === FOR Loop begin agr_tsk.agreement_id: '||agr_tsk.agreement_id);
            xx_au_debug.p_write(' === FOR Loop begin agr_tsk.task_id: '||agr_tsk.task_id);
          END IF;
          ...
          IF g1_debug_mode                  = 'Y' THEN
            xx_au_debug.p_write(' l_revamt_eve_crtd '||l_revamt_eve_crtd||' l_last_event_date '||l_last_event_date);
          END IF;
          l_num_of_mon                            :=0;
          ...
          xx_au_debug.p_write(' l_num_of_mon : '||TO_CHAR(l_num_of_mon));
          IF TO_CHAR(l_last_event_date,'YYYY-MM') = '9999-12' AND l_revamt_eve_crtd = 0 THEN
            IF g1_debug_mode                      = 'Y' THEN
              xx_au_debug.p_write('First Rev Event to be created for the project.');
            END IF;
          ...
          ELSE
            IF --l_num_of_mon = 0 AND
              ...
              IF g1_debug_mode                     = 'Y' THEN
                xx_au_debug.p_write(' Event already created for curr month, adj even to be crtd only. ');
                xx_au_debug.p_write(' chk_adj_eve_flag '||chk_adj_eve_flag||' crt_curr_mon_eve_flag '||crt_curr_mon_eve_flag);
              END IF;
            ELSE -- to_char(l_last_rev_eve_dt,'YYYY-MM') = to_char(l_last_event_date,'YYYY-MM')
              --l_curr_rev_eve_dt := last_day(l_last_event_date+1);
              IF g1_debug_mode = 'Y' THEN
                xx_au_debug.p_write(' Event to be created for curr month ');
                xx_au_debug.p_write(' Also chk for ADJ Event creation.');
              END IF;
              chk_adj_eve_flag      := 'Y';
          ...
          END IF;
          IF g1_debug_mode = 'Y' THEN
            xx_au_debug.p_write(' FINAL chk_adj_eve_flag '||chk_adj_eve_flag||' crt_curr_mon_eve_flag '||crt_curr_mon_eve_flag);
            xx_au_debug.p_write(' before loop start l_num_of_mon : '||l_num_of_mon);
          END IF;
          LOOP
            budget_revenue    :=0;
            IF l_num_of_mon    < 0 THEN
              IF g1_debug_mode = 'Y' THEN
                xx_au_debug.p_write(' before exit loop ');
              END IF;
              EXIT;
            ELSIF l_num_of_mon  >= 1 THEN
              ...
              IF g1_debug_mode   = 'Y' THEN
                xx_au_debug.p_write(' in loop new l_num_of_mon : '||l_num_of_mon||' l_curr_rev_eve_dt '||l_curr_rev_eve_dt);
              END IF;
            ELSE
              l_num_of_mon    := l_num_of_mon-1;
              IF g1_debug_mode = 'Y' THEN
            ...
            if l_curr_rev_eve_dt = l_last_event_date --and crt_curr_mon_eve_flag = 'Y'
             ...
             if l_last_mon_per_closed = 'Y' then
               ...
               if g1_debug_mode = 'Y' then
                 xx_au_debug.p_write(' As l_curr_rev_eve_dt = l_last_event_date thus crt_curr_mon_eve_flag is set from Y to N');
               end if;
              -- Fix RT 5058267
              if l_last_mon_per_closed != 'Y' then
               crt_curr_mon_eve_flag := 'N';
            ...
            WHEN no_data_found THEN
              IF g1_debug_mode = 'Y' THEN
                xx_au_debug.p_write(TO_CHAR(l_curr_rev_eve_dt)||' is not in any Open Period.');
                xx_au_debug.p_write(' Invalid X_rev_or_bill_date Param Value.');
              END IF;
            ...
            WHEN OTHERS THEN
              IF g1_debug_mode = 'Y' THEN
                xx_au_debug.p_write('Error from pa_billing_extn_params_v pa_rev_ca.Auto_rev_event_gen :'||SQLERRM);
              END IF;
              RAISE;
            END;
            l_no_days_accrued :=0;
            ...
            LOOP
              IF g1_debug_mode = 'Y' THEN
                xx_au_debug.p_write('  --- FOR fund_line LOOP project_funding_id '||TO_CHAR(fund_line.project_funding_id));
                xx_au_debug.p_write('  --- FOR fund_line LOOP date_allocated '||TO_CHAR(fund_line.date_allocated));
                xx_au_debug.p_write('  --- FOR fund_line LOOP allocated_amount '||TO_CHAR(fund_line.allocated_amount));
              END IF;
              -- Calc revenue per day amt
              l_rev_amt_per_day := fund_line.allocated_amount / l_prj_no_of_days;
              IF g1_debug_mode   = 'Y' THEN
                xx_au_debug.p_write(' l_rev_amt_per_day = '||l_rev_amt_per_day);
              end if;
              ...
              ELSE
                l_no_days_accrued := -1;
                IF g1_debug_mode   = 'Y' THEN
                  xx_au_debug.p_write(' ERROR l_no_days_accrued '||l_no_days_accrued);
                END IF;
                raise ERR_COUNT_DAYS;
              END IF;
              IF g1_debug_mode = 'Y' THEN
                xx_au_debug.p_write(' l_no_days_accrued '||l_no_days_accrued);
              END IF;
              l_rev_amt2           := 0;
              l_adj_rev_amt        :=0;
            ...
            END LOOP;
            IF g1_debug_mode = 'Y' THEN
              xx_au_debug.p_write(' Rev amt for which events sud have been be crtd till now : tot_eve_amt2 = '||tot_eve_amt2);
              xx_au_debug.p_write(' Actual Rev amt for which events are created till now : '||l_revamt_eve_crtd);
              xx_au_debug.p_write(' Date of last event crt l_last_event_date '||l_last_event_date);
            END IF;
            IF tot_eve_amt2 <> l_revamt_eve_crtd THEN
              l_adj_rev_amt := NVL(tot_eve_amt2,0) - NVL(l_revamt_eve_crtd,0);
            END IF;
            IF g1_debug_mode = 'Y' THEN
              xx_au_debug.p_write(' Adj amt calculated as l_adj_rev_amt : '||l_adj_rev_amt);
            END IF;
            ...
            IF g1_debug_mode = 'Y' THEN
              xx_au_debug.p_write('pa_billing_pub.insert_event IN Param tot_l_rev_amt:' ||tot_rev_eve_amt|| ' X_project_id:' ||X_project_id|| ' X_top_task_id: ' ||agr_tsk.task_id|| ' X_reference2: ' ||TO_CHAR(agr_tsk.agreement_id) );
            END IF;
            IF tot_rev_eve_amt <> 0 THEN
              pa_billing_pub.insert_event(x_project_id => x_project_id,
                                          ...
                                          X_status => err_status);
              IF g1_debug_mode = 'Y' THEN
                xx_au_debug.p_write(' Done Inserting REV event '||err_status);
              END IF;
              IF err_status     <> 0 THEN
                IF g1_debug_mode = 'Y' THEN
                  xx_au_debug.p_write(' ERROR from pa_billing_pub.insert_event for REV Amt');
                END IF;
            ...
            IF g1_debug_mode  = 'Y' THEN
            xx_au_debug.p_write('pa_billing_pub.insert_event IN Param l_adj_rev_amt:'||l_adj_rev_amt||' X_project_id:'||X_project_id);
            END IF;
            pa_billing_pub.insert_event(X_project_id        => X_project_id,
            ...
            X_status            => err_status);
            IF g1_debug_mode  = 'Y' THEN
            xx_au_debug.p_write(' Done Inserting ADJ event '||err_status);
            END IF;
            IF err_status <> 0 THEN
            IF g1_debug_mode  = 'Y' THEN
            xx_au_debug.p_write(' ERROR from pa_billing_pub.insert_event for ADJ Amt');
            END IF;
            raise ERR_INSERT_EVENT;
            END IF;
            end if;*/
  ...
  ELSE --l_prj_funding_lvl <> 'PROJECT'
    ...
    if l_prj_funding_lvl = 'TASK' then
      ...
      exception
         when OTHERS then
           xx_au_debug.p_write('Project '||X_PROJECT_ID||' Funding Level is TASK. ERROR fetching Work Type. NO EVENTS CEATED.'||sqlerrm);
    ...
    --   if L_WORK_TYPE_NM <> 'Straight Line Revenue' then   -- commented for CR22995
	  if NVL(lv_task_rev_rec_method, 'X') <> 'Straight Line Revenue' then      -- added for CR22995
        xx_au_debug.p_write('Project '||X_PROJECT_ID||' Funding Level is TASK, and Work Type for Task '||L_TOP_TASK_ID||' is not Straight Line Revenue. Thus, NO EVENTS CREATED.'||err_status);
       ...
       IF g1_debug_mode = 'Y' THEN
        ...
        LOOP
          ...
          AND NVL(BILL_TRANS_REV_AMOUNT,0) <> 0;
          if g1_debug_mode                  = 'Y' then
            xx_au_debug.p_write(' X_project_id '||X_project_id||' AGR_TSK.AGREEMENT_ID '||AGR_TSK.AGREEMENT_ID||' L_LOOKUP_FOUND '||L_LOOKUP_FOUND||' L_PRJ_FUNDING_LVL '||L_PRJ_FUNDING_LVL||' L_LOOKUP_TASK_ID '||L_LOOKUP_TASK_ID||' AGR_TSK.TASK_ID '||AGR_TSK.TASK_ID);
            xx_au_debug.p_write(' l_revamt_eve_crtd '||l_revamt_eve_crtd||' l_last_event_date '||l_last_event_date);
          END IF;
		  
		  BEGIN   --Start of INC0647629 (Logic to exit the loop if task total revenue is greater than revenue of particular set of events)
          SELECT sum(PROJECT_ALLOCATED_AMOUNT)
           into l_prjt_fund_amount
            FROM PA_PROJECT_FUNDINGS_V
            WHERE project_id     = X_project_id
            AND (task_id    =   DECODE(L_LOOKUP_FOUND,'Y',
                                                                 DECODE(L_PRJ_FUNDING_LVL,'PROJECT',
          ...
          fnd_file.put_line (fnd_file.LOG,' l_prjt_fund_amount '||l_prjt_fund_amount);
          
          IF l_revamt_eve_crtd >= l_prjt_fund_amount THEN
		  
          fnd_file.put_line (fnd_file.LOG,' l_prjt_fund_amount '||l_prjt_fund_amount||' l_revamt_eve_crtd '||l_revamt_eve_crtd);

          RAISE ERR_INVALID_BUDGET;
        END IF;
        END;   --End of INC0647629
            ...
            LOOP
              ...
              IF TO_CHAR(l_curr_rev_eve_dt,'YYYY-MM') = TO_CHAR(l_prj_end_dt,'YYYY-MM') THEN
                ...
                IF (chk_adj_eve_flag = 'Y' AND crt_curr_mon_eve_flag = 'N') THEN
                  ...
                  l_rev_amt  := 0;
                  if g1_debug_mode = 'Y' then
                    xx_au_debug.p_write(' 11 Logic to calculate the Rev Amt ');
                  END IF;
                ELSIF (CHK_ADJ_EVE_FLAG  = 'Y' AND CRT_CURR_MON_EVE_FLAG = 'Y') THEN
                  ...
                  if g1_debug_mode = 'Y' then
                    xx_au_debug.p_write(' 22 Logic to calculate the Rev Amt curr_eve_num_days: '||curr_eve_num_days);
                  END IF;
                  l_rev_amt            := l_rev_amt_per_day * curr_eve_num_days;
                elsif (chk_adj_eve_flag = 'F' and crt_curr_mon_eve_flag = 'F') then
                  curr_eve_num_days    := l_task_no_of_days; --(l_prj_end_dt - l_prj_start_dt) +1; CR22722
                  if g1_debug_mode = 'Y' then
                    xx_au_debug.p_write(' 33 Logic to calculate the Rev Amt curr_eve_num_days: '||curr_eve_num_days);
                  END IF;
                  l_rev_amt            := NVL(l_rev_amt_per_day,0) * NVL(curr_eve_num_days,0);
                END IF;
            --  ELSIF TO_CHAR(l_curr_rev_eve_dt,'YYYY-MM') = TO_CHAR(l_prj_start_dt,'YYYY-MM') THEN
			  ELSIF TO_CHAR(l_curr_rev_eve_dt,'YYYY-MM') = TO_CHAR(l_tsk_start_dt,'YYYY-MM') THEN -- Changed l_prj_start_dt to l_tsk_start_dt
                ...
                IF (chk_adj_eve_flag    = 'Y' AND crt_curr_mon_eve_flag = 'N') THEN
                  l_rev_amt            := 0;
                  if g1_debug_mode = 'Y' then
                    xx_au_debug.p_write(' 44 Logic to calculate the Rev Amt curr_eve_num_days: '||curr_eve_num_days);
                  END IF;
                elsif (chk_adj_eve_flag = 'Y' and crt_curr_mon_eve_flag = 'Y') and to_char(l_last_event_date,'YYYY-MM') <> '9999-12' then
                  ...
                  l_rev_amt            := l_rev_amt_per_day * curr_eve_num_days;
                  if g1_debug_mode = 'Y' then
                    xx_au_debug.p_write(' 55 Logic to calculate the Rev Amt curr_eve_num_days: '||curr_eve_num_days);
                  END IF;
                elsif (chk_adj_eve_flag = 'F' and crt_curr_mon_eve_flag = 'F') then
                  curr_eve_num_days    := (l_curr_rev_eve_dt       - l_tsk_start_dt) +1;  -- CR22722 l_prj_start_dt replaced with l_tsk_start_dt
                  l_rev_amt            := NVL(l_rev_amt_per_day,0) * NVL(curr_eve_num_days,0);
                  if g1_debug_mode = 'Y' then
                    xx_au_debug.p_write(' 66 Logic to calculate the Rev Amt curr_eve_num_days: '||curr_eve_num_days);
                  END IF;
                END IF;
              ELSE
                IF (chk_adj_eve_flag = 'Y' AND crt_curr_mon_eve_flag = 'N') THEN
                  l_rev_amt := 0;
                    ...
                    if to_char(fund_line.date_allocated,'YYYY-MM') <= to_char(l_curr_rev_eve_dt,'YYYY-MM')
                      ...
                      curr_eve_num_days := (l_curr_rev_eve_dt - l_tsk_start_dt)+1; -- CR22722 l_prj_start_dt replaced with l_tsk_start_dt
                      if g1_debug_mode = 'Y' then
                        xx_au_debug.p_write(' 77 Logic to calculate the Rev Amt curr_eve_num_days: '||curr_eve_num_days);
                      END IF;
                    ELSE
                      curr_eve_num_days := NVL((TRUNC(l_curr_rev_eve_dt) - TRUNC(l_last_event_date)),0);
                      if g1_debug_mode = 'Y' then
                        xx_au_debug.p_write(' 88 Logic to calculate the Rev Amt curr_eve_num_days: '||curr_eve_num_days);
                      END IF;
                    end if;
                    l_rev_amt            := l_rev_amt_per_day * curr_eve_num_days;
                elsif (chk_adj_eve_flag = 'F' and crt_curr_mon_eve_flag = 'F') then
				 ...
				 -- End of Defect#26090
                  ...
                  l_rev_amt            := NVL(l_rev_amt_per_day,0) * NVL(curr_eve_num_days,0);
                   if g1_debug_mode = 'Y' then
                      xx_au_debug.p_write(' 1010 Logic to calculate the Rev Amt curr_eve_num_days: '||curr_eve_num_days);
                    END IF;
                END IF;
              END IF;
              TOT_L_REV_AMT     := TOT_L_REV_AMT + L_REV_AMT;
  ...
  IF G1_DEBUG_MODE = 'Y' THEN
    xx_au_debug.p_write(' =====END : Params Auto_rev_event_gen X_request_id : '||X_request_id );
  END IF;
EXCEPTION
...
WHEN lookup_err THEN
  ...
  X2_revenue_amount      := NULL;
  X_rev_budget_type_code := NULL;
  PA_BILLING_PUB.INSERT_MESSAGE (X_INSERTING_PROCEDURE_NAME =>'xxpa_rev_ca.AUTO_REV_EVENT_GEN',
