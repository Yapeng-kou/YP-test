create or replace PACKAGE BODY      xxpa_rev_cn
  ...
  **12-10-2023     Rahaman Rawoof           CR25222B - Project Billing Extension for HVAC China Maintenance Contracts Which job value <120K AND Extended Warranty
  ************************************************************************************/
  /*****************************************************************************
  -- Global variables to store the attribute12 - 15 columns of
  -- pa_billing_extensions table
  ****************************************************************************/
  gv_ca_event_type        VARCHAR2(30);
  gv_ca_contra_event_type VARCHAR2(30);
  gv_ca_wip_event_type    VARCHAR2(30);
  gv_ca_budget_type       VARCHAR2(1) ;
  /***************************************************************************
...
IS
lv_insert_eligible varchar2(20):='Y';
  ...
  ln_tot_rev_eve_acc_amt  NUMBER;
  ln_tot_rev_amt          NUMBER;
  ...
  lv_agreement_currency_code VARCHAR2(100);
  lv_project_currency_code   VARCHAR2(100);
  ln_revenue_total           NUMBER;
  ln_cnt                     NUMBER;
BEGIN
  IF g1_debug_mode = 'Y' THEN
    ...
    IF l_prj_funding_lvl = 'TASK' THEN
      BEGIN
        SELECT work_type_id, 
		       start_DATE, 
			   completion_DATE, 
			  (completion_DATE - start_DATE)+1 tsk_num_of_days
         INTO l_work_type_id, 
			  l_tsk_start_dt, 
			  l_tsk_end_dt, 
			  l_task_no_of_days
    ...
    END IF;
       IF g1_debug_mode = 'Y' THEN
        ...
        IF g1_debug_mode = 'Y' THEN
          ...
          fnd_file.put_line (fnd_file.LOG,' Params X_rev_budget_type_code : '||l_rev_budget_type_code);
        END IF;
		fnd_file.put_line (fnd_file.LOG,' budget_revenue : '||budget_revenue);
		fnd_file.put_line (fnd_file.LOG,' g1_debug_mode : '||g1_debug_mode);
        IF budget_revenue  = 0 THEN
          IF g1_debug_mode = 'Y' THEN
            fnd_file.put_line (fnd_file.LOG,' No budget for the project ');
          END IF;
        ...
        LOOP
          ...
          LOOP
            ...
            IF l_num_of_mon    < 0 THEN
              IF g1_debug_mode = 'Y' THEN
                fnd_file.put_line (fnd_file.LOG,' before exit loop AFTER');
              END IF;
              EXIT;
            ELSIF l_num_of_mon  >= 1 THEN
              l_curr_rev_eve_dt := last_day(l_last_event_DATE+1);
            ...
            LOOP
              IF g1_debug_mode = 'Y' THEN
                ...
                fnd_file.put_line (fnd_file.LOG,'  --- FOR fund_line LOOP DATE_allocated '||TO_CHAR(fund_line.DATE_allocated));
                fnd_file.put_line (fnd_file.LOG,'  --- FOR fund_line LOOP allocated_amount '||TO_CHAR(fund_line.allocated_amount));
                fnd_file.put_line (fnd_file.LOG,'  --- FOR fund_line LOOP Attribute4 '||fund_line.Attribute4);
				fnd_file.put_line (fnd_file.LOG,'  --- FOR fund_line LOOP funding_currency_code '||fund_line.funding_currency_code);
				fnd_file.put_line (fnd_file.LOG,'  --- FOR fund_line LOOP project_currency_code '||fund_line.project_currency_code);
				fnd_file.put_line (fnd_file.LOG,'  --- FOR fund_line LOOP project_exchange_rate '||fund_line.project_exchange_rate);
			  END IF;
			  -- Calc revenue per day amt
              l_rev_amt_per_day := fund_line.allocated_amount / l_task_no_of_days; -- l_prj_no_of_days; CR22722
			  -- Added for CR25222B Start--
			  IF fund_line.funding_currency_code = fund_line.project_currency_code THEN				  
					l_rev_annum_amt := (fund_line.attribute4/(l_task_no_of_days+1))*365;	--CR25222B
			  ELSE			  
					l_rev_annum_amt := ((fund_line.attribute4*fund_line.project_exchange_rate)/(l_task_no_of_days+1))*365;	--CR25222			  
			  END IF;	
			  -- Added for CR25222B End--
              ...
              IF l_no_days_accrued >= 0 THEN
                ...
                --l_rev_amt2 := pa_currency.round_currency_amt(l_rev_amt2);
              END IF;
			  fnd_file.put_line (fnd_file.LOG,' l_rev_amt2 INSIDE l_no_days_accrued '||l_rev_amt2);
			  fnd_file.put_line (fnd_file.LOG,' tot_eve_amt2 '||tot_eve_amt2);
              tot_eve_amt2                           := tot_eve_amt2 + l_rev_amt2;
              /* Logic to calculate the Rev Amt */
              IF TO_CHAR(l_curr_rev_eve_dt,'YYYY-MM') = TO_CHAR(l_prj_end_dt,'YYYY-MM') THEN
                -- Calculating Revenue Amount for last event creation for the Project
            ...
            --  ELSIF TO_CHAR(l_curr_rev_eve_dt,'YYYY-MM') = TO_CHAR(l_prj_start_dt,'YYYY-MM') THEN
              ...
              ELSE
                ...
                END IF;
              END IF;
			  fnd_file.put_line (fnd_file.LOG,' L_REV_AMT : '||L_REV_AMT); 
              TOT_L_REV_AMT     := TOT_L_REV_AMT + L_REV_AMT;
              L_REV_AMT         := 0;
              L_REV_AMT_PER_DAY :=0;
              curr_eve_num_days :=0;
            ...
            IF g1_debug_mode = 'Y' THEN
              ...
              fnd_file.put_line (fnd_file.LOG,' Actual Rev amt for which events are created till now : '||l_revamt_eve_crtd);
              fnd_file.put_line (fnd_file.LOG,' DATE of last event crt l_last_event_DATE '||l_last_event_DATE);
			  fnd_file.put_line (fnd_file.LOG,' TOT_L_REV_AMT : '||TOT_L_REV_AMT); 
            END IF;
            ...
            tot_rev_eve_amt := tot_l_rev_amt+ l_adj_rev_amt;
			fnd_file.put_line (fnd_file.LOG,' tot_rev_eve_amt : '||tot_rev_eve_amt); 
            ...
            IF g1_debug_mode = 'Y' THEN
              fnd_file.put_line (fnd_file.LOG,'pa_billing_pub.insert_event IN Param tot_l_rev_amt:' ||tot_rev_eve_amt|| ' X_project_id:' ||X_project_id|| ' X_top_task_id: ' ||agr_tsk.task_id|| ' X_reference2: ' ||TO_CHAR(agr_tsk.agreement_id) );
            END IF;
			fnd_file.put_line (fnd_file.LOG,' test 5678');
			fnd_file.put_line (fnd_file.LOG,' l_meaning 1 : '||l_meaning); 
			fnd_file.put_line (fnd_file.LOG,' test 1234');
			fnd_file.put_line (fnd_file.LOG,' ln_tot_rev_amt  : '||ln_tot_rev_amt);
			fnd_file.put_line (fnd_file.LOG,' test 10');

				  		-- Added for CR25222B Start--
		    BEGIN
				SELECT NVL(SUM(NVL(allocated_amount,0)),0)
				  INTO ln_revenue_total
				  FROM PA_PROJECT_FUNDINGS_V
				 where project_id                         = X_project_id
			...
			EXCEPTION
                 WHEN OTHERS THEN
				    ln_revenue_total := 0;
            END;
			fnd_file.put_line (fnd_file.LOG,' ln_revenue_total : '||ln_revenue_total);
			  BEGIN
				SELECT SUM(total_accrued_amount) 
				   INTO ln_tot_rev_eve_acc_amt
                   From pa_summary_project_fundings
                   where project_id =  X_project_id
				   AND task_id 		= X_top_task_id; 
			  EXCEPTION
                 WHEN OTHERS THEN
				    ln_tot_rev_eve_acc_amt := NULL;
              END;
			  fnd_file.put_line (fnd_file.LOG,' ln_tot_rev_eve_acc_amt : '||ln_tot_rev_eve_acc_amt);

			  ln_curr_eve_num_days   := (X_rev_or_bill_DATE - l_tsk_start_dt)+1;
			  ln_total_days          := (l_tsk_end_dt - l_tsk_start_dt)+1;

			  fnd_file.put_line (fnd_file.LOG,' curr_eve_num_days1 : '||ln_curr_eve_num_days);
			  fnd_file.put_line (fnd_file.LOG,' ln_total_days : '||ln_total_days);

			  ln_tot_rev_amt := ((ln_curr_eve_num_days/ln_total_days)*ln_revenue_total)-ln_tot_rev_eve_acc_amt;
			  --ln_tot_rev_amt := (((l_curr_rev_eve_dt - l_tsk_start_dt+1)/(l_tsk_end_dt - l_tsk_start_dt +1))*ln_revenue_total)-ln_tot_rev_eve_acc_amt;

			  fnd_file.put_line (fnd_file.LOG,' l_curr_rev_eve_dt1 : '||l_curr_rev_eve_dt);
			  fnd_file.put_line (fnd_file.LOG,' l_tsk_start_dt1 : '||l_tsk_start_dt);
			  fnd_file.put_line (fnd_file.LOG,' ln_revenue_total : '||ln_revenue_total);
			  fnd_file.put_line (fnd_file.LOG,' l_tsk_end_dt : '||l_tsk_end_dt);			  
			  fnd_file.put_line (fnd_file.LOG,' ln_tot_rev_amt : '||ln_tot_rev_amt);
			  -- Added for CR25222B End --
            ...
            IF tot_rev_eve_amt <> 0 THEN
              ...
              fnd_file.put_line(fnd_file.log,'l_curr_rev_eve_dt:'||l_curr_rev_eve_dt);
              fnd_file.put_line(fnd_file.log,'X_rev_or_bill_DATE:'||X_rev_or_bill_DATE);

              IF l_meaning = 'Service Agreement' AND l_curr_rev_eve_dt = X_rev_or_bill_DATE THEN  -- Added for CR25222B added and condition
               fnd_file.put_line(fnd_file.log,'Test 2 ');
               IF l_rev_annum_amt < TO_NUMBER(l_job_value) THEN	-- Added for CR25222
						--Accrue through DATE Month = Task End DATE Month
                        fnd_file.put_line(fnd_file.log,'Test 3 ');
					IF TO_CHAR(l_curr_rev_eve_dt,'YYYY-MM') = TO_CHAR(l_tsk_end_dt,'YYYY-MM') 
					    AND l_last_month_hold = 'Yes' THEN  -- Added for CR25222		
                        fnd_file.put_line(fnd_file.log,'Test 4 ');
							fnd_file.put_line (fnd_file.LOG,' BEFORE pa_billing_pub '); 
				    pa_billing_pub.insert_event(x_project_id  	        => x_project_id,
											    x_top_task_id 		    => l_event_task_id,											  
											    x_rev_amt  			    => ROUND(ln_tot_rev_amt,2),
											    x_bill_amt 			    => 0,
											    x_event_type  		    => lv_event_type,
											    x_completion_DATE 	    => l_tsk_end_dt,
											    x_revenue_hold_flag     => l_hold_flag,
--                                                x_reference1 => fnd_global.conc_request_id,
											    x_reference2 			=> TO_CHAR(agr_tsk.agreement_id),
											    x_txn_currency_code 	=> l_eve_crt_curr_code,
											    x_error_message 		=> err_msg,
											    x_status 				=> err_status);
							...
							fnd_file.put_line (fnd_file.LOG,' ln_tot_rev_amt 1 : '||ln_tot_rev_amt);
                    ...
                    ELSE --TO_CHAR(l_curr_rev_eve_dt,'YYYY-MM') <> TO_CHAR(l_tsk_end_dt,'YYYY-MM')
                    fnd_file.put_line(fnd_file.log,'Test 5 ');
						fnd_file.put_line (fnd_file.LOG,' event created in ELSE part ');
                        fnd_file.put_line (fnd_file.LOG,' Project ID'||x_project_id);
						fnd_file.put_line (fnd_file.LOG,' l_event_task_id'||l_event_task_id);
                        fnd_file.put_line (fnd_file.LOG,' ln_tot_rev_amt '||ln_tot_rev_amt);
                        fnd_file.put_line (fnd_file.LOG,' l_curr_rev_eve_dt '||l_curr_rev_eve_dt);
                        fnd_file.put_line (fnd_file.LOG,' agreement_id '||TO_CHAR(agr_tsk.agreement_id));
                        fnd_file.put_line (fnd_file.LOG,' l_eve_crt_curr_code'||l_eve_crt_curr_code);
                        SELECT DECODE(COUNT(1), '0','Y','N')
                          INTO lv_insert_eligible
                          FROM pa_events
                         WHERE event_type 	= 'Service Revenue'
                           AND project_id 	= x_project_id
                           AND task_id 		= X_top_task_id
                           AND reference1 	= fnd_global.conc_request_id
                           AND reference2 	= TO_CHAR(agr_tsk.agreement_id)
                           AND bill_trans_rev_amount <> 0;

                        fnd_file.put_line(fnd_file.log,'Event Insert Eligble Flag = '||lv_insert_eligible);
                        IF lv_insert_eligible = 'Y' THEN 
                        ...
                        pa_billing_pub.insert_event(x_project_id  		    => x_project_id,
							...
							fnd_file.put_line (fnd_file.LOG,' err_status IN ELSE : '||err_status);
                            fnd_file.put_line (fnd_file.LOG,' err_msg IN ELSE : '||err_msg);

                        end if;
				END IF; --TO_CHAR(l_curr_rev_eve_dt,'YYYY-MM') = TO_CHAR(l_tsk_end_dt,'YYYY-MM')
				END IF;	--l_rev_annum_amt < TO_NUMBER(l_job_value)

						fnd_file.put_line (fnd_file.LOG,' AFTER pa_billing_pub ');	
						fnd_file.put_line (fnd_file.LOG,' l_meaning 2 : '||l_meaning);

				ELSIF l_meaning = 'Extended Warranty' AND l_curr_rev_eve_dt = X_rev_or_bill_DATE THEN  --l_meaning
					  fnd_file.put_line (fnd_file.LOG,' event created in elsIF part ');
                      fnd_file.put_line(fnd_file.log,'Test 6 ');
                  pa_billing_pub.insert_event(x_project_id  		=> x_project_id,
											  x_top_task_id 		=> l_event_task_id,
											  x_rev_amt  			=> ROUND(ln_tot_rev_amt,2),
											  x_bill_amt 			=> 0,
